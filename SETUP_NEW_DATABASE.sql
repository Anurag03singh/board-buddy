-- ============================================
-- TaskFlow Database Setup (Fixed Version)
-- ============================================
-- Run this in Supabase SQL Editor for project: wnlmuaoekxmeatkffoyu
...
-- FIXED: Board policies without recursion
CREATE POLICY "Board visible to owner"
...
-- FIXED: Board members policies without recursion
CREATE POLICY "Board members visible to participants"
...
CREATE POLICY "Lists visible to board owner"
  ON public.lists FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can insert lists"
  ON public.lists FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can update lists"
  ON public.lists FOR UPDATE TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can delete lists"
...
CREATE POLICY "Tasks visible to board owner"
  ON public.tasks FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can insert tasks"
  ON public.tasks FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can update tasks"
  ON public.tasks FOR UPDATE TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.boards WHERE id = board_id AND owner_id = auth.uid())
  );

CREATE POLICY "Board owner can delete tasks"
...
CREATE POLICY "Task assignments visible to board owner"
...
CREATE POLICY "Board owner can assign tasks"
...
CREATE POLICY "Board owner can unassign tasks"
...
CREATE POLICY "Activity visible to board owner"
...
CREATE INDEX IF NOT EXISTS idx_activity_log_board ON public.activity_log(board_id);
CREATE INDEX IF NOT EXISTS idx_tasks_title ON public.tasks USING gin(to_tsvector('english', title));

-- Success message
SELECT 'Database setup complete! All tables created with fixed RLS policies.' as status;
